...
def save_picture(result, tolerance):
    base_name = os.path.basename(original_path)
    name_and_extension = os.path.splitext(base_name)
    new_name = name_and_extension[0] + '_' + str(int(tolerance * 10)).zfill(2) + ".png"
    result_path = os.path.join(result_dir, new_name)
...
def print_result_array(array):
    for tolerance in tolerances:
        print("\t%.2f" % tolerance, end="")
    print()
    for row in range(array.shape[0]):
        picture_name = os.path.basename(pairs_of_images[row][0])
...
if __name__ == "__main__":
    ...
    result_dir = "C:\\Users\\Michal\\Pictures\\magisterka\\wykrywanie_podloza\\wyniki_tolerancja"
    max_depth = 20
    ...
    tolerances = [0.1, 0.3, 0.5, 0.7, 0.9, 1.1]
    ...
    not_found_ground_values = np.zeros((len(pairs_of_images), len(tolerances)))
    qualified_obstacles_values = np.zeros((len(pairs_of_images), len(tolerances)))

    for picture in enumerate(pairs_of_images):
        original_path = picture[1][0]
        ground_true_path = picture[1][1]
        ...
        for tolerance in enumerate(tolerances):
            ground_points_qualifier = GroundPointsQualifier(depth_generator.frame_shape, tolerance[1], curve_degree)
            ...
            not_found_ground_values[picture[0], tolerance[0]] = get_percentage(not_found, ground)
            qualified_obstacles_values[picture[0], tolerance[0]] = get_percentage(found_obst, obst)
            ...
            save_picture(presented_result, tolerance[1])
